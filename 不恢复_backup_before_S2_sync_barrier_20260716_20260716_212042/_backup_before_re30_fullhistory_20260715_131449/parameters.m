...
init_pi = init_pi_0 - a .* init_pesc;%初始能源共享价格
%% 
...
% 可行的约 30% RE 数值热启动（当前固定随机种子/物理实例标定）。
% 保持 pes=pesc=0，故社区等式、全局平衡与全部线路约束均从可行点开始；
% 仅将 pg 从原冷启动值向 pg_max 做固定比例混合，再由 pb/ps 补足 EU 功率平衡。
% 标定参考为当前实例 exactUpperObj=25940.8260425，得到初始目标 33723.0738552，
% 即 RE=30%%。它不是前一时段状态，也不得与冷启动结果混合报告。
init_mode = 'feasible_pzero_re30_calibrated';
init_target_RE = 0.30;
init_reference_obj = 25940.8260425;
init_pg_mix = 0.35302056767788;
init_pg_cold = min(max(D + init_pes, pg_min), pg_max);
init_pg = (1 - init_pg_mix) .* init_pg_cold + init_pg_mix .* pg_max;
...
save('params.mat','a','b','c','D','start_idx','end_idx', ...
    ...
    'init_mode','init_target_RE','init_reference_obj','init_pg_mix','init_pg_cold', ...
