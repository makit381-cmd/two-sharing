...
outer_momentum = 0.02;
inner_momentum = 0.1;
% 有限安全上限：上层/内层需以此值预分配历史数组，不能设为 Inf 或 9e100。
max_inner_iter = 5e4;
min_inner_iter = 20;
inner_cv_tol_kW = 100; % 内层遍历平均 raw Agg--EU CV 停止阈值
inner_avg_start_iter = 5; % 前4步 burn-in；第5步起统计正式内层后缀平均输出
max_outer_iter = 5e4;
min_outer_iter = 10;
outer_cv_tol_kW = 300; % 外层遍历平均 PB/线路联合 CV 停止阈值
outer_avg_start_iter = 5;
record_inner_comm = 71;
record_k = 50:70;
% 每次 S3 仅覆盖固定结果文件，不再以时间戳新增历史 MAT。
save_s3_results = true;
run_tag = ['s3_kw_rawagg_nomomentum_', char(datetime('now','Format','yyyyMMdd_HHmmss_SSS'))];
result_unit = 'kW';
