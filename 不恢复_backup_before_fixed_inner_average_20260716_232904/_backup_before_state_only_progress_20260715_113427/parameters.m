...
max_inner_iter = 5e4;
min_inner_iter = 20;
inner_cv_ratio = 0.01; % 每个社区相对自身绝对负荷的 1%
inner_avg_start_iter = 10; % 前4步 burn-in；第5步起统计正式内层后缀平均输出
max_outer_iter = 5e4;
min_outer_iter = 10;
outer_cv_ratio = 0.01; % 实时口径：总平衡与每条有限线路分别限制在 1%
outer_avg_start_iter = 10;
...
% 冷启动必须满足每个产消者的本地功率平衡；也供 upperMarketS3 读取。
...
save('params.mat','a','b','c','D','start_idx','end_idx', ...
    ...
    'min_inner_iter','inner_cv_ratio','community_load_scale_kW','inner_cv_tol_kW','inner_avg_start_iter', ...
    'max_outer_iter','min_outer_iter','outer_cv_ratio','system_balance_scale_kW','outer_pb_cv_tol_kW','outer_line_cv_tol_kW','cv_policy', ...
    'outer_avg_start_iter','record_inner_comm','record_k','save_s3_results','run_tag','result_unit', ...
