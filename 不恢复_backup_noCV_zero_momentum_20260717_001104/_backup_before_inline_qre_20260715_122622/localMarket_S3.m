...
while true
    ...
    if progress_print_enabled && (h == 1 || mod(h, progress_print_inner_every) == 0)
        ...
        fprintf('[S3 inner] k=%d, community=%d, h=%d, %s=%.3f kW, diagnostic limit=%.3f kW, lambdaStep=%.3e\n', ...
    ...
    % 对应 Algorithm 2 S1.3：只以 Agg 对偶变量的更新步长停止。
    % 因采用工程性 burn-in，至少需已有一个后缀平均输出才允许返回。
    if h >= max(min_inner_iter, inner_avg_start_iter) && lambda_step <= errTol
        res.stop_reason = 'price_step';
        break;
    end
    if h >= max_inner_iter
        res.success = 0;
...
res.inner_cv_stop_enabled = false;
res.iter = h;
res.price_updates = h;
