...
val_obj = nan(A,1);
val_obj(1) = communityPrimalObj(val_pg(:,1), val_pb(:,1), ...
...
while true
    ...
    if record_trace
        ...
        % 保留产生本步行为响应的瞬时 lambda，供 QRE 瞬时诊断使用。
        val_lambda_used(h+1) = val_lambda(h);
        % 正式内层 RE/CV 均针对同一份遍历平均社区输出。
        val_obj(h+1) = communityPrimalObj(bar_pg, bar_pb, bar_ps, ...
            bar_pes, bar_pesc_raw);
    end
