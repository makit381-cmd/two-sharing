...
% formal/test 参数分离：pilot 只通过环境变量限制本次有效外层更新数，
% 不改写正式 params.mat。
formal_max_outer_price_updates = max_outer_price_updates;
formal_outer_avg_start_price_update = outer_avg_start_price_update;
runtime_cap_text = getenv('HDEM_QRE_S3_MAX_OUTER_UPDATES');
runtime_cap = str2double(runtime_cap_text);
test_overrides = struct('outer_price_update_cap',runtime_cap, ...
    'outer_avg_start_price_update',NaN, ...
    'checkpoint_every_outer_updates',NaN, ...
    'pilot_runtime_budget_s',NaN, ...
    'resume_from_checkpoint',false, ...
    'checkpoint_file','');
if isfinite(runtime_cap) && runtime_cap > 0
    max_outer_price_updates = min(max_outer_price_updates,floor(runtime_cap));
    max_outer_iter = max_outer_price_updates;
end
runtime_checkpoint_every = str2double(getenv('HDEM_QRE_S3_CHECKPOINT_EVERY'));
if isfinite(runtime_checkpoint_every) && runtime_checkpoint_every > 0
    checkpoint_every_outer_updates = floor(runtime_checkpoint_every);
    test_overrides.checkpoint_every_outer_updates = checkpoint_every_outer_updates;
end
checkpoint_dir = getenv('HDEM_QRE_S3_CHECKPOINT_DIR');
if isempty(checkpoint_dir), checkpoint_dir = pwd; end
resume_requested = strcmp(getenv('HDEM_QRE_S3_RESUME'),'1');
checkpoint_path_requested = getenv('HDEM_QRE_S3_CHECKPOINT');
if resume_requested
    if isempty(checkpoint_path_requested) || ~isfile(checkpoint_path_requested)
        error('upperMarketS3:checkpointMissing','Resume checkpoint does not exist.');
    end
    test_overrides.resume_from_checkpoint = true;
    test_overrides.checkpoint_file = checkpoint_path_requested;
end
runtime_tag = getenv('HDEM_QRE_S3_RUN_TAG');
if ~isempty(runtime_tag)
    run_tag = [run_tag '_' runtime_tag];
end
runtime_budget_s = str2double(getenv('HDEM_QRE_S3_MAX_RUNTIME_SECONDS'));
if ~(isfinite(runtime_budget_s) && runtime_budget_s > 0)
    runtime_budget_s = Inf;
end
if isfinite(runtime_budget_s)
    test_overrides.pilot_runtime_budget_s = runtime_budget_s;
end
run_start_tic = tic;
...
% localMarket_S3 返回字段随审计版本扩展，使用空 struct 动态接收，
% 避免手工模板遗漏字段并避免为所有社区预分配大型嵌套结构。
res = struct([]);
if ~isempty(res)
res(num_LESMs) = struct('success',[],'pi',[],'pes',[],'pg',[], ...
    ...
    'qre_audit_enabled',[],'qre_audit_rate',[],'qre_audit_count',[], ...
...
total_inner_wall_iter = 0;
num_local_call = 0;
max_inner_wall_iter_observed = 0;
max_inner_price_updates_observed = 0;
total_local_time = 0;
local_call_runtime_history = zeros(0,1);
outer_update_runtime_history = zeros(0,1);
local_failure_hold_count = 0;
local_failure_hold_by_community = zeros(num_LESMs,1);
local_failure_stop_reason_history = cell(0,1);
outer_trace = struct();
outer_trace.pb_residual_before_kW = nan(outer_history_capacity,1);
outer_trace.pb_residual_after_kW = nan(outer_history_capacity,1);
outer_trace.piPB_before = nan(outer_history_capacity,1);
outer_trace.piPB_after = nan(outer_history_capacity,1);
outer_trace.piPB_step = nan(outer_history_capacity,1);
outer_trace.norm_pi0_before = nan(outer_history_capacity,1);
outer_trace.norm_pi0_after = nan(outer_history_capacity,1);
outer_trace.active_community_count = zeros(outer_history_capacity,1);
outer_trace.held_community_count = zeros(outer_history_capacity,1);
outer_trace.sum_active_new_pesc = nan(outer_history_capacity,1);
outer_trace.sum_active_old_pesc = nan(outer_history_capacity,1);
outer_trace.sum_inactive_held_pesc = nan(outer_history_capacity,1);
outer_trace.line_flow_before = nan(numel(F_l),outer_history_capacity);
outer_trace.line_flow_after = nan(numel(F_l),outer_history_capacity);
outer_trace.line_positive_violation_before = nan(numel(F_l),outer_history_capacity);
outer_trace.line_positive_violation_after = nan(numel(F_l),outer_history_capacity);
outer_trace.line_negative_violation_before = nan(numel(F_l),outer_history_capacity);
outer_trace.line_negative_violation_after = nan(numel(F_l),outer_history_capacity);
outer_trace.max_violation_line_id = nan(outer_history_capacity,1);
outer_trace.max_violation_direction = zeros(outer_history_capacity,1);
outer_trace.max_violation_before_kW = nan(outer_history_capacity,1);
outer_trace.max_violation_after_kW = nan(outer_history_capacity,1);
outer_trace.max_line_flow_before = nan(outer_history_capacity,1);
outer_trace.max_line_flow_after = nan(outer_history_capacity,1);
outer_trace.max_line_pi_pos_before = nan(outer_history_capacity,1);
outer_trace.max_line_pi_pos_after = nan(outer_history_capacity,1);
outer_trace.max_line_pi_neg_before = nan(outer_history_capacity,1);
outer_trace.max_line_pi_neg_after = nan(outer_history_capacity,1);
outer_trace.line_price_component_norm = nan(outer_history_capacity,1);
...
outer_avg_cnt = 0;
recent_outer_price_steps = nan(outer_price_stable_window,1);
recent_outer_price_count = 0;
recent_outer_price_position = 0;
recent_outer_pb_pass = false(outer_formal_cv_stable_window,1);
recent_outer_line_pass = false(outer_formal_cv_stable_window,1);
recent_outer_cv_count = 0;
recent_outer_cv_position = 0;
outer_price_stable_pass = false;
outer_pb_cv_stable_pass = false;
outer_line_cv_stable_pass = false;
...
if resume_requested
    requested_runtime_cap = runtime_cap;
    requested_checkpoint_every = checkpoint_every_outer_updates;
    requested_test_overrides = test_overrides;
    load(checkpoint_path_requested,'-mat');
    if exist('checkpoint_next_k','var')
        k = checkpoint_next_k;
    end
    % 恢复后仍以本次命令给出的 cap/effective 参数为准。
    runtime_cap = requested_runtime_cap;
    checkpoint_every_outer_updates = requested_checkpoint_every;
    test_overrides = requested_test_overrides;
    resume_requested = true;
    if isfinite(runtime_cap) && runtime_cap > 0
        max_outer_price_updates = min(formal_max_outer_price_updates,floor(runtime_cap));
        max_outer_iter = max_outer_price_updates;
    end
end
...
while true
    ...
    if k + 1 > outer_history_capacity
        ...
        traceNames = fieldnames(outer_trace);
        for traceIndex = 1:numel(traceNames)
            traceValue = outer_trace.(traceNames{traceIndex});
            if isvector(traceValue)
                traceValue(nextCapacity,1) = NaN;
            else
                traceValue(:,nextCapacity) = NaN;
            end
            outer_trace.(traceNames{traceIndex}) = traceValue;
        end
    ...
    pesc_before_slot = pesc;
    local_called_mask = false(num_LESMs,1);
    for i = 1:num_LESMs
        ...
        if count(i) < delay(i)
            ...
            local_result = localMarket_S3(c(start_idx(i):end_idx(i)), ...
            ...
            if isempty(res)
                res = repmat(local_result,num_LESMs,1);
            else
                res(i) = local_result;
            end

            local_result_success = logical(res(i).success);
            if ~local_result_success && ~strcmp(res(i).stop_reason,'max_inner_iter')
                error('upperMarketS3:localFailure', ...
                    'Community %d failed: %s', i, res(i).stop_reason);
            end
            if local_result_success && qre_certificate_enabled && ~res(i).qre_all_certificate_pass
                error('upperMarketS3:qreCertificateFailure', ...
                    'Community %d returned an uncertified QRE response.', i);
            end
            if local_result_success && agg_certificate_enabled && ~res(i).agg_certificate_pass
            ...
            local_call_runtime_history(end+1,1) = local_elapsed;
            count(i) = 0;
            delay(i) = randi([0,k0(i)]);
            if local_result_success
                % 只有成功完成本次局部更新，才发布新的 held 状态和 raw Agg。
                % max_inner_iter 失败在实时异步语义下保持上一次上传，不上传未认证截断值。
                pi(i) = res(i).pi;
                ...
                local_failure_hold_count = local_failure_hold_count + 1;
                local_failure_hold_by_community(i) = local_failure_hold_by_community(i) + 1;
                local_failure_stop_reason_history{end+1,1} = res(i).stop_reason;
            end
            % 记录时，RE 与 CV 均使用同一份遍历平均社区输出。
            if local_result_success && record_inner_or_not
    ...
    active_community_count = nnz(local_called_mask);
    held_community_count = num_LESMs - active_community_count;
    pb_residual_before_kW = sum(pesc_before_slot);
    pb_residual_after_kW = sum(pesc);
    sum_active_new_pesc = sum(pesc(local_called_mask));
    sum_active_old_pesc = sum(pesc_before_slot(local_called_mask));
    sum_inactive_held_pesc = sum(pesc_before_slot(~local_called_mask));
    line_flow_before = pai_il' * pesc_before_slot;
    line_flow_after = pai_il' * pesc;
    line_positive_violation_before = max(line_flow_before - F_l,0);
    line_positive_violation_after = max(line_flow_after - F_l,0);
    line_negative_violation_before = max(-line_flow_before - F_l,0);
    line_negative_violation_after = max(-line_flow_after - F_l,0);

    % S3 每个 wall slot 都完成一次 held 外层价格更新；保留两个显式计数。
    outer_price_update_count = outer_price_update_count + 1;

    %% HDEM式上层运行平均：从第一个真实外层价格更新起计入输出。
    if outer_price_update_count >= outer_avg_start_price_update
    ...
    line_price_component = pai_il * (val_pi_l_pos(:,k+1) - val_pi_l_neg(:,k+1));
    all_violation_before = [line_positive_violation_before(finite_line_mask); ...
        line_negative_violation_before(finite_line_mask)];
    all_violation_after = [line_positive_violation_after(finite_line_mask); ...
        line_negative_violation_after(finite_line_mask)];
    [max_violation_before_kW,max_before_idx] = max(all_violation_before);
    [max_violation_after_kW,~] = max(all_violation_after);
    numFinite = nnz(finite_line_mask);
    if isempty(max_before_idx) || max_violation_before_kW <= 0
        max_violation_line_id = NaN;
        max_violation_direction = 0;
        max_line_flow_before = NaN;
        max_line_flow_after = NaN;
        max_line_pi_pos_before = NaN;
        max_line_pi_pos_after = NaN;
        max_line_pi_neg_before = NaN;
        max_line_pi_neg_after = NaN;
    else
        if max_before_idx <= numFinite
            max_violation_direction = 1;
            finitePosition = max_before_idx;
        else
            max_violation_direction = -1;
            finitePosition = max_before_idx - numFinite;
        end
        max_violation_line_id = finite_line_idx(finitePosition);
        max_line_flow_before = line_flow_before(max_violation_line_id);
        max_line_flow_after = line_flow_after(max_violation_line_id);
        max_line_pi_pos_before = val_pi_l_pos(max_violation_line_id,k);
        max_line_pi_pos_after = val_pi_l_pos(max_violation_line_id,k+1);
        max_line_pi_neg_before = val_pi_l_neg(max_violation_line_id,k);
        max_line_pi_neg_after = val_pi_l_neg(max_violation_line_id,k+1);
    end
    outer_trace.pb_residual_before_kW(k) = pb_residual_before_kW;
    outer_trace.pb_residual_after_kW(k) = pb_residual_after_kW;
    outer_trace.piPB_before(k) = val_pi_PB(1,k);
    outer_trace.piPB_after(k) = val_pi_PB(1,k+1);
    outer_trace.piPB_step(k) = val_pi_PB(1,k+1) - val_pi_PB(1,k);
    outer_trace.norm_pi0_before(k) = norm(val_pi_0(:,k));
    outer_trace.norm_pi0_after(k) = norm(val_pi_0(:,k+1));
    outer_trace.active_community_count(k) = active_community_count;
    outer_trace.held_community_count(k) = held_community_count;
    outer_trace.sum_active_new_pesc(k) = sum_active_new_pesc;
    outer_trace.sum_active_old_pesc(k) = sum_active_old_pesc;
    outer_trace.sum_inactive_held_pesc(k) = sum_inactive_held_pesc;
    outer_trace.line_flow_before(:,k) = line_flow_before;
    outer_trace.line_flow_after(:,k) = line_flow_after;
    outer_trace.line_positive_violation_before(:,k) = line_positive_violation_before;
    outer_trace.line_positive_violation_after(:,k) = line_positive_violation_after;
    outer_trace.line_negative_violation_before(:,k) = line_negative_violation_before;
    outer_trace.line_negative_violation_after(:,k) = line_negative_violation_after;
    outer_trace.max_violation_line_id(k) = max_violation_line_id;
    outer_trace.max_violation_direction(k) = max_violation_direction;
    outer_trace.max_violation_before_kW(k) = max_violation_before_kW;
    outer_trace.max_violation_after_kW(k) = max_violation_after_kW;
    outer_trace.max_line_flow_before(k) = max_line_flow_before;
    outer_trace.max_line_flow_after(k) = max_line_flow_after;
    outer_trace.max_line_pi_pos_before(k) = max_line_pi_pos_before;
    outer_trace.max_line_pi_pos_after(k) = max_line_pi_pos_after;
    outer_trace.max_line_pi_neg_before(k) = max_line_pi_neg_before;
    outer_trace.max_line_pi_neg_after(k) = max_line_pi_neg_after;
    outer_trace.line_price_component_norm(k) = norm(line_price_component);
    ...
    recent_outer_price_count = min(recent_outer_price_count + 1, numel(recent_outer_price_steps));
    recent_outer_price_position = mod(recent_outer_price_position, numel(recent_outer_price_steps)) + 1;
    recent_outer_price_steps(recent_outer_price_position) = outer_price_step(k);
    outer_price_stable_pass = recent_outer_price_count >= numel(recent_outer_price_steps) && ...
        all(recent_outer_price_steps <= errTol_UESM);
    recent_outer_cv_count = min(recent_outer_cv_count + 1, numel(recent_outer_pb_pass));
    recent_outer_cv_position = mod(recent_outer_cv_position, numel(recent_outer_pb_pass)) + 1;
    recent_outer_pb_pass(recent_outer_cv_position) = outer_pb_cv_pass;
    recent_outer_line_pass(recent_outer_cv_position) = outer_line_cv_pass;
    outer_pb_cv_stable_pass = recent_outer_cv_count >= numel(recent_outer_pb_pass) && ...
        all(recent_outer_pb_pass);
    outer_line_cv_stable_pass = recent_outer_cv_count >= numel(recent_outer_line_pass) && ...
        all(recent_outer_line_pass);
    outer_update_runtime_history(k,1) = toc(outer_update_tic);
    % checkpoint 保存整个脚本工作区，覆盖价格、held 状态、平均累加器、
    % local-call 计数、随机配置、历史与停止窗口；next_k 防止恢复时重复当前轮。
    if isfinite(checkpoint_every_outer_updates) && ...
            checkpoint_every_outer_updates > 0 && ...
            mod(outer_price_update_count,checkpoint_every_outer_updates) == 0
        checkpoint_next_k = k + 1;
        checkpoint_run_tag = run_tag;
        checkpoint_file = fullfile(checkpoint_dir, sprintf( ...
            'S3_checkpoint_%s_update_%05d.mat',checkpoint_run_tag,outer_price_update_count));
        save(checkpoint_file);
        latest_checkpoint_file = checkpoint_file;
    end

    if toc(run_start_tic) >= runtime_budget_s
        state.success = 0;
        outer_stop_reason = 'pilot_runtime_budget';
        break;
    end

    outer_cv_stable_gate = (~logical(outer_cv_stop_enabled)) || ...
        (outer_pb_cv_stable_pass && outer_line_cv_stable_pass);
    if outer_price_update_count >= max(min_outer_iter, outer_avg_start_price_update) && ...
            outer_avg_cnt > 0 && outer_price_stable_pass && outer_cv_stable_gate
    ...
    if isfinite(max_outer_price_updates) && ...
            outer_price_update_count >= max_outer_price_updates
        state.success = 0;
        outer_stop_reason = 'max_outer_price_updates';
        break;
    end
...
raw_agg_objective_gap_abs = abs(totcost_iter - totcost_ref);
raw_agg_objective_gap_pct = RE_totcost;
...
outer.RE_totcost = RE_totcost; % 兼容旧字段；不是无条件严格 centralized RE。
outer.RE_totcost_legacy = RE_totcost;
outer.raw_agg_objective_gap_abs = raw_agg_objective_gap_abs;
outer.raw_agg_objective_gap_pct = raw_agg_objective_gap_pct;
outer.reference_objective_coupling = ...
    'centralized_reference_enforces_pesc_equals_sum_pes';
outer.objective_gap_interpretation = ...
    'raw_agg_output_objective_gap_diagnostic_not_strict_RE';
outer.CV_l2_history_kW = violation_iter;
% 异步保持的 k 本身就是全局墙钟；补齐与 S0/S2 相同的绘图字段。
outer.wall_k = (1:outer_wall_iter)';
...
outer.wall_price_update_count = (1:outer_price_update_count)';
outer.wall_update_event = true(outer_wall_iter,1);
...
outer.outer_history_semantics = ['each k is a global wall-clock event: completed communities upload new suffix-average ', ...
    'raw Agg and unfinished communities hold their last upload; average starts at the specified real price update; ', ...
    'RE/CV are diagnostics, not unqualified strict centralized RE'];
...
outer.outer_avg_start_price_update = outer_avg_start_price_update;
outer.stable_outer_window = stable_outer_window;
outer.stable_outer_price_count = stable_outer_price_count;
outer.outer_price_stable_window = outer_price_stable_window;
outer.outer_formal_cv_stable_window = outer_formal_cv_stable_window;
outer.outer_price_stable_pass = outer_price_stable_pass;
outer.outer_pb_cv_stable_pass = outer_pb_cv_stable_pass;
outer.outer_line_cv_stable_pass = outer_line_cv_stable_pass;
...
outer.outer_price_update_count = outer_price_update_count;
outer.outer_wall_iter = outer_wall_iter;
outer.formal_max_outer_price_updates = formal_max_outer_price_updates;
outer.effective_max_outer_price_updates = max_outer_price_updates;
outer.formal_outer_avg_start_price_update = formal_outer_avg_start_price_update;
outer.test_overrides = test_overrides;
outer.checkpoint_every_outer_updates = checkpoint_every_outer_updates;
outer.resume_from_checkpoint = resume_requested;
if exist('latest_checkpoint_file','var')
    outer.latest_checkpoint_file = latest_checkpoint_file;
else
    outer.latest_checkpoint_file = '';
end
...
outer.local_failure_hold_count = local_failure_hold_count;
outer.local_failure_hold_by_community = local_failure_hold_by_community;
outer.local_failure_stop_reason_history = local_failure_stop_reason_history;
outer.local_failure_semantics = ['max_inner_iter is treated as a held-update failure: ', ...
    'the previous community state remains uploaded; the partial local result is not published'];
...
outer.total_inner_wall_iter = total_inner_wall_iter;
...
outer.local_call_runtime_history = local_call_runtime_history;
outer.outer_update_runtime_history = outer_update_runtime_history(1:outer_price_update_count);
traceNames = fieldnames(outer_trace);
for traceIndex = 1:numel(traceNames)
    traceValue = outer_trace.(traceNames{traceIndex});
    if isvector(traceValue)
        outer_trace.(traceNames{traceIndex}) = traceValue(1:outer_price_update_count);
    else
        outer_trace.(traceNames{traceIndex}) = traceValue(:,1:outer_price_update_count);
    end
end
outer.outer_trace = outer_trace;
outer.runtime_budget_s = runtime_budget_s;
outer.elapsed_runtime_s = toc(run_start_tic);
