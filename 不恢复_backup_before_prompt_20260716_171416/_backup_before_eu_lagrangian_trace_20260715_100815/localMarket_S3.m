function res = localMarket_S3(c,b,a,D,init_rho,pg_max,pg_min,init_pi, ...
    ...
    inner_momentum,max_inner_iter,h0,min_inner_iter,qre_certificate_enabled,agg_certificate_enabled)
...
while true
    ...
    bar_pesc_raw = sum_pesc / avg_cnt;%社区运营者对偶分解之后的子问题求解
    % Algorithm 2 原始 Agg 遍历输出；不得用 EU 输出之和覆盖上传量。
    ...
    if record_trace
        ...
        val_obj(h+1) = originalLowerObj(bar_pg, bar_pb, bar_ps, bar_pes, bar_pesc_raw);
    end
