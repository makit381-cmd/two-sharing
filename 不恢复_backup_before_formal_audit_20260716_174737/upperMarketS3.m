...
res(num_LESMs) = struct('success',[],'pi',[],'pes',[],'pg',[], ...
    ...
    'qre_all_certificate_pass',[],'qre_response_count',[],'agg_certificate_enabled',[], ...
...
while true
    ...
    if k >= max(min_outer_iter, outer_avg_start_iter) && ...
        ...
        if logical(outer_cv_stop_enabled)
            outer_stop_reason = 'stable_price_and_average_l2_cv';
        else
...
outer.raw_l2_cv_diagnostic_only = false;
