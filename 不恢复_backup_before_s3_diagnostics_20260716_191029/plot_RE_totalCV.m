...
wall_field = [field_name '_wall'];
if isfield(out,wall_field) && isfield(out,'wall_k')
    value = out.(wall_field)(:);
    k = out.wall_k(:);
else
    warning(['%s lacks wall-clock history. This MAT uses the old price-update ', ...
        'index and cannot be compared with the revised S0--S3 plot. Rerun its scenario.'], field_name);
