function res = localMarket_S3(c,b,a,D,init_rho,pg_max,pg_min,init_pi, ...
    ...
    outer_iter,community_id,rng_config,avg_config)
...
outer_key = 0;
if isfinite(outer_iter), outer_key = max(0,floor(outer_iter)); end
community_key = 0;
if isfinite(community_id), community_key = max(0,floor(community_id)); end
seed_modulus = 2^32 - 1;
inner_delay_seed = mod(rng_config.inner_delay_seed + 1009 * outer_key + ...
    1000003 * community_key,seed_modulus);
qre_noise_seed = mod(rng_config.qre_noise_seed + 1009 * outer_key + ...
    1000003 * community_key,seed_modulus);
qre_audit_seed_effective = mod(rng_config.qre_audit_seed + 1009 * outer_key + ...
    1000003 * community_key,seed_modulus);
