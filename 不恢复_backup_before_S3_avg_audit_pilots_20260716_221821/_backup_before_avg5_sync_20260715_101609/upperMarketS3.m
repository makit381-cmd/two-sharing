...
load('params.mat','a','b','c','D','start_idx','end_idx', ...
    ...
    'min_inner_iter','inner_cv_tol_kW', ...
...
res(num_LESMs) = struct('success',[],'pi',[],'pes',[],'pg',[], ...
    ...
    'inner_cv_kW',[],'xj2',[],'iter',[],'price_updates',[],'stop_reason',[], ...
...
val_xj2(:,1) = init_xj2;
%% 开始迭代
outer_price_step = nan(max_outer_iter + 2, 1);
over_trace = 0;
inner_k = struct('h',[], ...
    'val_violation',[], ...
    'val_obj',[], ...
...
outer_avg_cnt = 0;
val_totcost_bar = nan(max_outer_iter + 2, 1);
val_outer_violation_bar = nan(max_outer_iter + 2, 1);
    ...
    for i = 1:num_LESMs
        ...
        if count(i) < delay(i)
            ...
            res(i) = localMarket_S3(c(start_idx(i):end_idx(i)), ...
                ...
                min_inner_iter, qre_certificate_enabled, agg_certificate_enabled);
            ...
            if record_inner_or_not
                lambda_trace = res(i).trace_lambda_used(:);
                obj_ref = exactEUDualObjFast( ...
                ...
                inner_k.record_source = 'own_inner_final_h';
...
outer.qre_calls = sum([res.qre_calls]);
outer.qre_perturbed_accepts = sum([res.qre_perturbed_accepts]);
outer.qre_backoff_steps = sum([res.qre_backoff_steps]);
outer.qre_fallbacks = sum([res.qre_fallbacks]);
outer.qre_boundary_hits = sum([res.qre_boundary_hits]);
outer.qre_fallback_rate = outer.qre_fallbacks / max(outer.qre_calls, 1);
outer.qre_boundary_hit_rate = outer.qre_boundary_hits / max(outer.qre_calls, 1);
outer.qre_max_gap = NaN;
outer.qre_epsilon = NaN;
outer.qre_max_backoffs = qre_max_backoffs;
outer.qre_certificate_enabled = false;
outer.qre_profile = 'unchecked_qre';
