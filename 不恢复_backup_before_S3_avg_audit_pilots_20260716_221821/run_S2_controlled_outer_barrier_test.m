function summary = run_S2_controlled_outer_barrier_test(max_price_updates)
% 受控 S2 外层同步屏障测试。
...
for cycle = 1:max_price_updates
    ...
    if any(cycle_call_counts(cycle,:) ~= 1) || ...
            ...
            cycle_avg_counts_after(cycle) ~= cycle_avg_counts_before(cycle) + 1
        barrier_pass = false;
    end
...
count_relation_pass = all(all(cycle_call_counts == 1)) && ...
    ...
    price_update_count == max_price_updates && outer_avg_cnt == max_price_updates;
summary = struct();
