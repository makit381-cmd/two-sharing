...
use_ergodic_average = false; % S1：不作外层遍历平均。
...
res(num_LESMs) = struct('success',[],'pi',[],'pes',[],'pg',[], ...
    ...
    'inner_cv_kW',[],'inner_avg_cnt',[],'inner_avg_start_iter',[],'inner_cv_stop_enabled',[],'iter',[],'price_updates',[],'stop_reason',[], ...
    'trace_violation',[],'trace_h',[],'trace_obj',[],'trace_lambda',[],'trace_lambda_used',[], ...
    'trace_price_update_count',[],'trace_update_event',[]);
...
while true
    ...
    if progress_print_enabled && (k == 1 || mod(k, progress_print_outer_every) == 0)
        fprintf('[S1 outer start] k=%d, completed local calls=%d\n', k, num_local_call);
    end
    for i = 1:num_LESMs
        ...
        if count(i) < delay(i)
            ...
            local_tic = tic;
            res(i) = localMarket_S1(c(start_idx(i):end_idx(i)), ...
    ...
    %% S1 不作上层遍历平均：保留本轮瞬时 raw Agg 输出。
    if use_ergodic_average && k >= outer_avg_start_iter
    ...
    if progress_print_enabled && (k == 1 || mod(k, progress_print_outer_every) == 0)
        ...
        fprintf('[S1 outer] k=%d, PBavg=%.3f/%.3f kW, maxLineCV=%.4f, L2diag=%.3f kW\n', ...
        ...
        fprintf('[S1 line] k=%d, inst(line %d)=%.4f, avg(line %d)=%.4f\n', ...
...
outer.outer_history_semantics = ['each k is a global wall-clock event: completed communities upload new raw Agg ', ...
    'and unfinished communities hold their last upload; RE/CV are diagnostics, not stop criteria'];
...
outer.line_diagnostic_semantics = ['instantaneous=raw Agg upload at outer k; ', ...
    'average=instantaneous raw Agg upload (no averaging); dual=post-update multiplier at k'];
...
if save_s3_results
    % 用户指定：S1 每次仅覆盖同一对固定文件，避免累积时间戳结果。
    inner_result_file = 'inner_k_data_1.mat';
    outer_result_file = 'outer_data_1.mat';
