...
max_inner_iter = inf;
min_inner_iter = 0;
inner_cv_ratio = 0.02; % 每个社区相对自身绝对负荷的 2%
% 当前残差受限配置：CV 同时作为内层停止条件与诊断指标。
inner_cv_stop_enabled = true;
% 工程性后缀平均：前 199 步仅作 burn-in，h=200 起统计上传输出。
inner_avg_start_iter = 200;
% 不使用人为最小/最大外层次数；由价格步长和平均残差共同决定停止。
max_outer_iter = inf;
min_outer_iter = 0;
outer_cv_ratio = 0.02; % 总平衡与每条有限线路分别限制在 2%
% 当前残差受限配置：CV 同时作为外层停止条件与诊断指标。
outer_cv_stop_enabled = true;
% 工程性后缀平均：前 49 个外层更新仅作 burn-in，k=50 起统计上传输出。
outer_avg_start_iter = 50;
...
run_tag = ['s3_kw_rawagg_mout0p02_min0p10_', char(datetime('now','Format','yyyyMMdd_HHmmss_SSS'))];
result_unit = 'kW';
...
% 固定随机种子下的可行约 30% RE 数值初值。
% 随机 pg 端点与逐社区和为零的随机 pes 方向经同一混合系数缩放；
% 因此 EU 功率平衡、Agg--EU 等式、总平衡及初始线路流均保持可行。
% 标定参考为当前实例 exactUpperObj=25940.8260425，得到初始目标 33723.0738552，
% 即 RE=30%%。该档仅用于数值初始化，不是 QRE 响应或前一时段真实状态。
init_mode = 'random_feasible_re30_calibrated';
init_target_RE = 0.30;
init_reference_obj = 25940.8260425;
init_random_seed = 20260715;
init_random_pes_span_kW = 10;
init_random_mix = 0.51631707720117;
init_pg_cold = min(max(D + init_pes, pg_min), pg_max);
rng(init_random_seed,'twister');
init_pg_random = pg_min + rand(num_prosumers,1) .* (pg_max - pg_min);
init_pes_random = zeros(num_prosumers,1);
for i = 1:num_LESMs
    idx = start_idx(i):end_idx(i);
    direction_i = init_random_pes_span_kW .* (2 .* rand(numel(idx),1) - 1);
    init_pes_random(idx) = direction_i - mean(direction_i);
end
init_pg = (1 - init_random_mix) .* init_pg_cold + init_random_mix .* init_pg_random;
init_pes = init_random_mix .* init_pes_random;
...
alpha_l = alpha_PB;
%% 内外层容错
% 停止仍仅检查价格变化；50 kW 仅用于将价格门槛按各社区步长缩放。
inner_price_scale_kW = 100;
...
save('params.mat','a','b','c','D','start_idx','end_idx', ...
    ...
    'init_mode','init_target_RE','init_reference_obj','init_random_seed','init_random_pes_span_kW','init_random_mix','init_pg_cold', ...
