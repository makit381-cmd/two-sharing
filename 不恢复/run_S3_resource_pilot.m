function pilot_summary = run_S3_resource_pilot(pilot_outer_price_updates,pilot_runtime_budget_s)
...
if nargin < 2 || isempty(pilot_runtime_budget_s)
    pilot_runtime_budget_s = 180;
end
validateattributes(pilot_outer_price_updates,{'numeric'}, ...
    {'scalar','real','finite','positive','integer'});
validateattributes(pilot_runtime_budget_s,{'numeric'}, ...
...
test_overrides = struct( ...
    ...
    'checkpoint_every_outer_updates',checkpoint_every, ...
    'pilot_runtime_budget_s',pilot_runtime_budget_s, ...
...
setenv('HDEM_QRE_S3_MAX_RUNTIME_SECONDS',num2str(pilot_runtime_budget_s));
...
pilot_summary.stop_reason = outer.stop_reason;
pilot_summary.local_failure_hold_count = outer.local_failure_hold_count;
pilot_summary.runtime_budget_s = pilot_runtime_budget_s;
...
pilot_summary.success_gate = outer.outer_price_update_count == pilot_outer_price_updates && ...
    ...
    ~strcmp(outer.stop_reason,'pilot_runtime_budget');
...
fprintf('S3 resource pilot P%d: %s, %.2f s, %d local calls, %d checkpoints\n', ...
    ...
    function clearPilotEnvironment()
        ...
        setenv('HDEM_QRE_S3_CHECKPOINT','');
        setenv('HDEM_QRE_S3_MAX_RUNTIME_SECONDS','');
    end
