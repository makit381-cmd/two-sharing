...
% S0 内层：与 S3 相同的 raw Agg/QRE/动态记录写法；不调用公共算法函数。
% 瞬时 raw 残差只用于 lambda 更新；正式 CV/RE 使用遍历平均输出。

N = length(b);
use_ergodic_average = false; % S0：不作遍历平均，正式输出即本轮 raw Agg。
...
if qre_certificate_enabled || agg_certificate_enabled
    error('localMarket_S0 is currently the unchecked_qre profile; enable certificates only after restoring the certified implementation.');
end
...
if progress_print_enabled
    fprintf('[S0 inner start] k=%d, community=%d, EU=%d, CV diagnostic reference=%.3f kW\n', ...
...
while true
    ...
    if ~all(ready)
        if record_trace
            ...
            trace_lambda(trace_count) = lambda;
            trace_lambda_used(trace_count) = lambda;
            trace_price_update_count(trace_count) = h - 1;
        end
        continue;
    end
    ...
    if use_ergodic_average && h >= inner_avg_start_iter
        avg_cnt = avg_cnt + 1;
    ...
    if record_trace
        ...
        trace_price_update_count(trace_count) = h;
    ...
    if progress_print_enabled && (h == 1 || mod(h, progress_print_inner_every) == 0)
        ...
        fprintf('[S0 inner] k=%d, community=%d, h=%d, %s=%.3f kW, diagnostic limit=%.3f kW, lambdaStep=%.3e\n', ...
    ...
    % 当前配置下 CV 仅作诊断；本情形无遍历平均。
    % 不设人为迭代下限；S0 不使用 inner_avg_start_iter。
...
if progress_print_enabled
    fprintf('[S0 inner end] k=%d, community=%d, h=%d, CVavg=%.3f kW, reason=%s\n', ...
