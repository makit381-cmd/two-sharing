...
% S1 内层：与 S3 相同的 raw Agg/QRE/动态记录写法；不调用公共算法函数。
% 瞬时 raw 残差只用于 lambda 更新；正式 CV/RE 使用遍历平均输出。

N = length(b);
use_ergodic_average = false; % S1：不作遍历平均，正式输出即本轮 raw Agg。
...
if qre_certificate_enabled || agg_certificate_enabled
    error('localMarket_S1 is currently the unchecked_qre profile; enable certificates only after restoring the certified implementation.');
end
...
while true
    ...
    if progress_print_enabled && (h == 1 || mod(h, progress_print_inner_every) == 0)
        ...
        fprintf('[S1 inner] k=%d, community=%d, h=%d, %s=%.3f kW, diagnostic limit=%.3f kW, lambdaStep=%.3e\n', ...
    ...
    % 不设人为迭代下限；S1 不使用 inner_avg_start_iter。
...
if progress_print_enabled
    fprintf('[S1 inner end] k=%d, community=%d, h=%d, CVavg=%.3f kW, reason=%s\n', ...
