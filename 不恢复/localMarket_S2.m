function res = localMarket_S2(c,b,a,D,init_rho,pg_max,pg_min,init_pi, ...
    ...
    pi_0,pi_max,pi_min,record_trace,beta_qre,qre_epsilon,~,qre_z_cap,~, ...
...
max_inner_wall_iter = (h0 + 1) * max_inner_price_updates + 100;
...
if ~isfield(rng_config,'qre_audit_enabled'), rng_config.qre_audit_enabled = true; end
if ~isfield(rng_config,'qre_audit_rate'), rng_config.qre_audit_rate = 1; end
...
qre_audit_seed = deriveDeterministicSeed(rng_config.qre_audit_seed, ...
    community_key,local_call_count,3);
delay_stream = RandStream('mt19937ar','Seed',inner_delay_seed);
qre_stream = RandStream('mt19937ar','Seed',qre_noise_seed);
audit_stream = RandStream('mt19937ar','Seed',qre_audit_seed);
...
qre_gap_sum = 0;
qre_gap_max = 0;
qre_gap_exceed_count = 0;
qre_gap_samples = zeros(0,1);
...
if progress_print_enabled
    fprintf('[S2 inner start] k=%d, community=%d, EU=%d, CV diagnostic reference=%.3f kW\n', ...
        outer_iter, community_id, N, inner_cv_tol_kW);
end

while true
    wall_iter = wall_iter + 1;
    % wall_iter 只作安全上限；正式收敛/最大 price update 判定仍只能在
    % 本轮真实同步更新完成后执行。这样等待状态不会被误记为一次更新。
    if isfinite(max_inner_wall_iter) && wall_iter > max_inner_wall_iter
        res.success = 0;
        res.stop_reason = 'max_inner_wall_iter';
        break;
    end
    lambda_cycle = lambda;
    ...
    for j = 1:N
        ...
        if count(j) < delay(j)
            ...
            qre = solveFastQREResponse(c(j),b(j),a,D(j),pg_min(j),pg_max(j), ...
                lambda_cycle,pi_max,pi_min,beta_qre(j),qre_z_cap, ...
                rng_config.qre_noise_enabled,qre_stream);
            fast_pg(j) = qre.pg;
            fast_pes(j) = qre.pes;
            fast_pb(j) = qre.pb;
            fast_ps(j) = qre.ps;
            audit_due = logical(rng_config.qre_audit_enabled) && ...
                (rand(audit_stream,1) <= rng_config.qre_audit_rate);
            if audit_due
                exact = solveExactEUResponse(c(j),b(j),a,D(j),pg_min(j),pg_max(j), ...
                    lambda_cycle,pi_max,pi_min);
                gap = max(qre.obj - exact.obj,0);
                qre_audit_count = qre_audit_count + 1;
                qre_audit_response_count = qre_audit_response_count + 1;
                qre_gap_sum = qre_gap_sum + gap;
                qre_gap_max = max(qre_gap_max,gap);
                qre_gap_exceed_count = qre_gap_exceed_count + (gap > qre_epsilon);
                if numel(qre_gap_samples) < 10000
                    qre_gap_samples(end+1,1) = gap;
                else
                    qre_gap_samples(mod(qre_audit_count - 1,10000) + 1) = gap;
                end
            end
            ready(j) = true;
        end
    ...
    % 关闭 CV 停止时，formal CV 稳定窗口也只能作为诊断，不能阻塞价格停止。
    inner_cv_stable_gate = (~logical(inner_cv_stop_enabled)) || formal_cv_stable_pass;
    if price_update_count >= max(min_inner_iter,1) && ...
            averaging_started && avg_cnt >= avg_config.min_samples && ...
            stable_price_pass && inner_cv_stable_gate && inner_cv_pass
    ...
    if isfinite(max_inner_price_updates) && ...
            price_update_count >= max_inner_price_updates
        res.success = 0;
        res.stop_reason = 'max_inner_price_updates';
        break;
    end
    if isfinite(max_inner_wall_iter) && wall_iter >= max_inner_wall_iter
        res.success = 0;
...
res.max_inner_price_updates = max_inner_price_updates;
res.max_inner_wall_iter = max_inner_wall_iter;
res.total_eu_responses = eu_response_count;
res.max_observed_EU_age = h0;
res.age_bound_h0 = h0;
res.age_bound_pass_h0 = true;
res.qre_audit_enabled = logical(rng_config.qre_audit_enabled);
res.qre_audit_count = qre_audit_count;
res.qre_audit_response_count = qre_audit_response_count;
res.qre_gap_max = qre_gap_max;
res.qre_gap_mean = qre_gap_sum / max(qre_audit_count,1);
if isempty(qre_gap_samples)
    res.qre_gap_p95 = NaN;
else
    res.qre_gap_p95 = prctile(qre_gap_samples,95);
end
res.qre_gap_exceed_count = qre_gap_exceed_count;
res.qre_gap_exceed_rate = qre_gap_exceed_count / max(qre_audit_count,1);
res.qre_all_certificate_pass = qre_gap_exceed_count == 0;
res.held_cv_kW = sync_cv_kW;
res.final_sync_cv_kW = sync_cv_kW;
