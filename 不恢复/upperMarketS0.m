...
use_ergodic_average = false; % S0：不作外层遍历平均。
...
while true
    ...
    if progress_print_enabled && (k == 1 || mod(k, progress_print_outer_every) == 0)
        fprintf('[S0 outer start] k=%d, completed local calls=%d\n', k, num_local_call);
    end
    for i = 1:num_LESMs
        ...
        if count(i) < delay(i)
            ...
            local_tic = tic;
            res(i) = localMarket_S0(c(start_idx(i):end_idx(i)), ...
    ...
    if progress_print_enabled && (k == 1 || mod(k, progress_print_outer_every) == 0)
        ...
        fprintf('[S0 outer] k=%d, PBavg=%.3f/%.3f kW, maxLineCV=%.4f, L2diag=%.3f kW\n', ...
        ...
        fprintf('[S0 line] k=%d, inst(line %d)=%.4f, avg(line %d)=%.4f\n', ...
...
outer.outer_history_semantics = ['outer k counts completed synchronous price updates; wall_* records every ', ...
    'global clock with held published raw-Agg output during waiting; RE/CV are diagnostics, not stop criteria'];
...
if save_s3_results
    % 用户指定：S0 每次仅覆盖同一对固定文件，避免累积时间戳结果。
    inner_result_file = 'inner_k_data_0.mat';
    outer_result_file = 'outer_data_0.mat';
