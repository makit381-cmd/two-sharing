...
sync_cnt = 0;
sync_price_step = nan(max(1000,1),1);
...
outer_wait_steps = nan(max_outer_iter, 1);
...
metric_ps = ps;
metric_pesc = pesc;
val_totcost_iter = nan(max_outer_iter, 1);
val_outer_violation_iter = nan(max_outer_iter, 1);
...
while true
    ...
    count = delay;
    for i = 1:num_LESMs
        record_inner_or_not = ismember(k,record_k) && ...
        (i == record_inner_comm) && (count(i) == delay(i)) && ~over_trace;
        if ~mask(i)
            if count(i) == delay(i)
                local_tic = tic;
    ...
    val_totcost_iter(k) = sum(0.5 .* c .* metric_pg.^2 ...
        + b .* metric_pg ...
        + pi_max .* metric_pb ...
        - pi_min .* metric_ps ...
        + 0.5 .* a_extend .* metric_pes.^2) ...
        + sum(0.5 .* a .* metric_pesc.^2);

    F_l_MW_bar = F_l / 1e3;
    y_bar_now_MW = metric_pesc / 1e3;
...
output_pes  = metric_pes;
output_pg   = metric_pg;
output_pb   = metric_pb;
output_ps   = metric_ps;
output_pesc = metric_pesc;
...
save('inner_k_data_0.mat','inner_k');
save('outer_data_0.mat','outer');
toc;
